...
public class Screen {
	...
	public Screen(int[][] m, int mapW, int mapH, TextureManager tm, int w, int h) {
		map = m;
		mapWidth = mapW;
		mapHeight = mapH;
		this.tm = tm;
	...
	public int[] update(Camera camera, int[] pixels) {
	    ...
	    for(int x=0; x<width; x=x+1) {
		    ...
		    int texX = (int)(wallX * (tm.getTexture(texNum).SIZE));
		    if(side == 0 && rayDirX > 0) {
		    ...
		    for(int y=drawStart; y<drawEnd; y++) {
		    	...
		    	int color;
		    	if(side == 0) {
					color = tm.getTexture(texNum).pixels[texX + (texY * tm.getTexture(texNum).SIZE)];
				} 
		    	else {
					color = (tm.getTexture(texNum).pixels[texX + (texY * tm.getTexture(texNum).SIZE)]>>1) & 8355711;  //Make y sides darker, but why????
				}
